...
**Game**
There is a Red Guy ('#') trying to make it from the start of a grid to the end. Control the Player ('P') using the arrow keys and place SPACEBAR to place down a temporary barricade to block the Red Guy's path\*, which is visualized using dots. You have a limited number of barricades, and they only last so long; try to keep the Red Guy from reaching the end for as long as possible.
